all: testa_velha.cpp   velha.cpp velha.hpp velha.o
	g++ -std=c++11 -Wall velha.o testa_velha.cpp -o testa_romanos
	./testa_romanos
	#use comentario se necessario

compile: testa_velha.cpp   velha.cpp velha.hpp velha.o
	g++ -std=c++11 -Wall velha.o testa_velha.cpp -o testa_romanos
	
test: testa_romanos	
	./testa_romanos
	
cpplint: testa_velha.cpp   velha.cpp velha.hpp
	cpplint   --exclude=catch.hpp  *.*
	
gcov: testa_velha.cpp   velha.cpp velha.hpp 
	g++ -std=c++11 -Wall -Wall -fprofile-arcs -ftest-coverage -c velha.cpp
	g++ -std=c++11 -Wall -fprofile-arcs -ftest-coverage velha.o testa_velha.cpp -o testa_romanos
	./testa_romanos
	gcov *.cpp	
	
debug: testa_velha.cpp   velha.cpp velha.hpp 
	g++ -std=c++11 -Wall -Wall -g -c velha.cpp
	g++ -std=c++11 -Wall  -g velha.o testa_velha.cpp -o testa_romanos
	gdb testa_romanos
	
	
cppcheck: testa_velha.cpp   velha.cpp velha.hpp
	cppcheck  --enable=warning . 

valgrind: testa_romanos
	valgrind --leak-check=yes --log-file=valgrind.rpt testa_romanos

velha.o : velha.cpp velha.hpp
	g++ -std=c++11 -Wall -Wall -c velha.cpp
	
testa_romanos: 	testa_velha.cpp   velha.cpp velha.hpp velha.o
	g++ -std=c++11 -Wall velha.o testa_velha.cpp -o testa_romanos

clean:
	rm -rf *.o *.exe *.gc* 
	
	
